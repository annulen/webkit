...
macro(macro_process_qtbase_prl_file qt_target_component)
    if (TARGET ${qt_target_component})
        ...
        if (EXISTS ${_prl_file_location})
            ...
            if (APPLE)
                string(REPLACE "-framework " "-framework--" _static_depends ${static_depends})
                string(REPLACE " " ";" _static_depends ${_static_depends})
                foreach (_curr_dep ${_static_depends})
                    if ("${_curr_dep}" MATCHES "^-framework--")
                        string(REPLACE "-framework--" "" _framework_name ${_curr_dep})
                        string(TOUPPER ${_framework_name} _library_prefix)
                        find_library(${_library_prefix}_LIBRARY "${_framework_name}")
                        if (${_library_prefix}_LIBRARY)
                            string(REPLACE "-framework ${_framework_name}" "${${_library_prefix}_LIBRARY}" static_depends ${static_depends})
                        else ()
                            string(REPLACE "-framework ${_framework_name}" "" static_depends ${static_depends})
                        endif ()
                    endif ()
                endforeach ()
            endif ()
